...
module apb_master_bfm (apb_if.master apb);
    ...
    task automatic apb_read(input [7:0] addr, output [31:0] data);
        ...
        @(apb.cb_master);  //access phase
        apb.cb_master.penable <= 1'b1;
        do @(apb.cb_master); while (!apb.cb_master.pready); //wait for slave in DUT
        if (apb.cb_master.pslverr !== 1'b0) begin
            $display("[ERROR] APB Read from addr %0h returned PSLVERR=1", addr);
        end
